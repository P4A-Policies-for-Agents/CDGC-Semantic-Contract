...
/// `https://example.com/data-use` work as written. A string is used as-is; a
/// single-element array yields that element; a number is stringified. Multi-element
/// arrays and objects return `None` — declared-use/acceptance/collection-id are
/// expected to be single-valued claims.
...
mod tests {
    ...
    fn decodes_and_reads_claims() {
        let jwt = make_jwt(&json!({"data_use":"analytics","sub":"u-1"}));
        let auth = format!("Bearer {jwt}");
        let claims = decode_bearer_claims(Some(&auth)).unwrap();
        assert_eq!(claim_str(&claims, "data_use").as_deref(), Some("analytics"));
        assert_eq!(claim_str(&claims, "sub").as_deref(), Some("u-1"));
        assert_eq!(claim_str(&claims, "missing"), None);
    }

    #[test]
    fn namespaced_claim_key_and_lowercase_scheme() {
        let jwt = make_jwt(&json!({"https://acme/data-use":"training"}));
        let auth = format!("bearer {jwt}");
        let claims = decode_bearer_claims(Some(&auth)).unwrap();
        assert_eq!(claim_str(&claims, "https://acme/data-use").as_deref(), Some("training"));
    }

    #[test]
    fn single_element_array_number_and_bool() {
        let claims = json!({"roles":["analytics"],"lvl":3,"accepted":true,"many":["a","b"]});
        assert_eq!(claim_str(&claims, "roles").as_deref(), Some("analytics"));
        assert_eq!(claim_str(&claims, "lvl").as_deref(), Some("3"));
        assert_eq!(claim_str(&claims, "accepted").as_deref(), Some("true"));
